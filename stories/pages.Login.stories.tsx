...
stories.add('No files', () => {
  ...
  specs(() => describe('No files', () => {
    it('clicking new should open create dialog', () => {
      ...
      expect(props.showCreateDialog.calledOnce).toBeTruthy()
    })
  }))
...
stories.add('One file', () => {
  ...
  specs(() => describe('One file', () => {
    it('clicking item should open login dialog', () => {
      ...
      expect(open).toExist()
      open.simulate('click')
      expect(props.showLoginDialog.calledOnce).toBeTruthy()
      expect(props.showLoginDialog.firstCall.args[0].info).toEqual(files[0])
    })
  }))
  ...
  specs(() => describe('Multiple files', () => {
    it('clicking items should open correct login dialog', () => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        expect(open).toExist()
        open.simulate('click')
        expect(props.showLoginDialog.callCount).toEqual(i + 1)
        expect(props.showLoginDialog.lastCall.args[0].info).toEqual(files[i])
      }
    })
  }))
